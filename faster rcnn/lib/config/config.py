...
tf.app.flags.DEFINE_integer('batch_size', 512, "Network batch size during training")
tf.app.flags.DEFINE_integer('max_iters', 20000, "Max iteration")
tf.app.flags.DEFINE_integer('step_size', 50, "Step size for reducing the learning rate, currently only support one step")
...
tf.app.flags.DEFINE_integer('snapshot_iterations', 20000, "Iteration to take snapshot")
...
tf.app.flags.DEFINE_float('roi_bg_threshold_high', 0.3, "Overlap threshold for a ROI to be considered background (class = 0 if overlap in [LO, HI))")
